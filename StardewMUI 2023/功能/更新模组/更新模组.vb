...
Public Class 更新模组
    ...
    Public Shared Function 生成更新地址表菜单() As 暗黑菜单条控件本体
        ...
        For i = 0 To 管理模组.当前项信息_N网ID列表.Count - 1
            ...
            AddHandler a.Items.Add("NEXUS: " & 管理模组.当前项信息_N网ID列表(i), My.Resources.NEXUS).Click, Sub(s, e) Process.Start(New ProcessStartInfo("https://www.nexusmods.com/stardewvalley/mods/" & Mid(s.Text, 8)) With {.UseShellExecute = True})
        ...
        For i = 0 To 管理模组.当前项信息_ModDropID列表.Count - 1
            AddHandler a.Items.Add("ModDrop: " & 管理模组.当前项信息_ModDropID列表(i), My.Resources.ModDrop_White32).Click, Sub(s, e) Process.Start(New ProcessStartInfo("https://www.moddrop.com/stardew-valley/mods/" & Mid(s.Text, 10)) With {.UseShellExecute = True})
        ...
        For i = 0 To 管理模组.当前项信息_Github仓库列表.Count - 1
            AddHandler a.Items.Add(管理模组.当前项信息_Github仓库列表(i), My.Resources.Github).Click, Sub(s, e) Process.Start(New ProcessStartInfo("https://github.com/" & s.Text) With {.UseShellExecute = True})
    ...
    Public Shared Sub 转到浏览器获取额外参数(模组ID As String, 文件ID As String, 模组项绝对路径 As String, Optional 结束后切换到选项卡 As String = "")
        ...
        ' 清掉上一轮残留的页面数据，避免计时器拿旧模组的参数发起下载
        浏览器WebView2控制.获取到的HTML数据 = ""
...
参数获取程序会一直运行，直到成功拿到参数或者手动点击右上角的取消"
    ...
    Public Shared Async Sub 获取服务器列表(模组ID As String, 文件ID As String, 模组项绝对路径 As String, Optional key As String = "", Optional expires As String = "", Optional 结束后切换到选项卡 As String = "")
        ...
        If str1 <> "" Then
            ...
            Form1.Panel34.Enabled = True
            If 批量更新队列.批量更新进行中 Then 批量更新队列.外部报告当前项失败("获取 NEXUS 服务器列表失败：" & str1, 模组项绝对路径)
            Exit Sub
        End If
        If a.name.Length = 0 Then
            Form1.Label34.Text = "   获取成功但 NEXUS API 没有返回任何可用的服务器"
            Form1.Panel34.Enabled = True
            If 批量更新队列.批量更新进行中 Then 批量更新队列.外部报告当前项失败("NEXUS API 没有返回任何可用的服务器", 模组项绝对路径)
            Exit Sub
        End If
        Form1.Panel34.Enabled = True
        If 设置.全局设置数据("AutoSelectFirstNexusDownloadSever") = "True" OrElse 批量更新队列.批量更新进行中 Then
    ...
    Public Shared Sub 添加到下载队列(下载地址 As String, 模组项绝对路径 As String, Optional 来源 As String = "nexus", Optional 自定义文件名 As String = "", Optional 结束后切换到选项卡 As String = "")
        ...
        Dim DW As New 下载进度界面块控件本体 With {.设置_下载来源 = 来源, .设置_下载地址 = 下载地址, .设置_模组项绝对路径 = 模组项绝对路径, .设置_N网模组ID = 正在处理的NEXUSID, .设置_其他来源指定文件名 = 自定义文件名, .Dock = DockStyle.Top, .设置_结束后切换到选项卡 = 结束后切换到选项卡, .设置_校验UniqueID = If(批量更新队列.批量更新进行中, 批量更新队列.当前项期望UniqueID, "")}
...
此状态会一直保持，直到文件被拖入或者手动点击右上角的取消"
    ...
    Public Shared Async Sub 获取Github文件列表(仓库作者和名称 As String, 模组项绝对路径 As String, Optional 结束后切换到选项卡 As String = "")
        ...
        For i = 0 To a.发行版数据集合.Count - 1
            ...
            Dim ix = i
            AddHandler 标题文字.LinkClicked, Sub() Process.Start(New ProcessStartInfo("https://github.com/" & 仓库作者和名称 & "/releases/tag/" & a.发行版数据集合(ix).标签) With {.UseShellExecute = True})
