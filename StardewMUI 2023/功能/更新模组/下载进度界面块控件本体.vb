...
Public Class 下载进度界面块控件本体
    ...
    ''' <summary>每个下载块走到终点时发出（安装成功、评估不通过、下载失败、解压失败、取消操作），批量更新队列依赖此信号推进</summary>
    Public Shared Event 单项更新流程结束(模组项绝对路径 As String, 是否成功 As Boolean, 结束说明 As String)

    ''' <summary>确保每个下载块在其生命周期内只发出一次结束信号，避免重复推进队列</summary>
    Private 是否已发出结束信号 As Boolean = False

    Private Sub 报告流程结束(是否成功 As Boolean, Optional 说明 As String = "")
        If 是否已发出结束信号 Then Exit Sub
        是否已发出结束信号 = True
        RaiseEvent 单项更新流程结束(设置_模组项绝对路径, 是否成功, 说明)
    End Sub
    ...
    ''' <summary>期望的模组 UniqueID（批量更新从步骤三条目带入）。解压后与下载内容的 manifest 比对，
    ''' 不一致拒绝安装，是多文件模组自动更新误选文件的最后一道防线；留空则跳过校验</summary>
    Public Property 设置_校验UniqueID As String = ""


    Public Async Sub 开始下载()
        If 设置_下载来源 = "" Then
            ' ModDrop 等来源是直接拖入已下载好的文件，跳过下载直接进入解压
            If 保存位置 <> "" AndAlso FileIO.FileSystem.FileExists(保存位置) Then
                Me.Label1.Text = If(FileIO.FileSystem.FileExists(Path.Combine(设置_模组项绝对路径, "Code2")), "更新：", "创建：") & Path.GetFileName(设置_模组项绝对路径)
                Me.Label2.Text = "已获取本地文件，直接开始解压"
                开始解压()
            End If
            Exit Sub
        End If
        ...
        Else
            是否下载成功 = True
            DebugPrint($"[NEXUS调试] 下载完成, 保存位置: {保存位置}, 扩展名: '{Path.GetExtension(保存位置)}'", Color1.蓝色)
            If String.IsNullOrEmpty(Path.GetExtension(保存位置)) AndAlso FileIO.FileSystem.FileExists(保存位置) Then
                DebugPrint($"[NEXUS调试] 文件无扩展名，开始检测魔数...", Color1.蓝色)
                Using fs As New FileStream(保存位置, FileMode.Open, FileAccess.Read)
                    Dim header(5) As Byte
                    Dim readCount As Integer = 0
                    While readCount < 6
                        Dim n = fs.Read(header, readCount, 6 - readCount)
                        If n = 0 Then Exit While
                        readCount += n
                    End While
                    Dim headerStr = String.Join(" ", header.Take(readCount).Select(Function(b) b.ToString("X2")))
                    DebugPrint($"[NEXUS调试] 文件头魔数: {headerStr}  文件大小: {fs.Length} bytes", Color1.蓝色)
                    Dim ext As String = ""
                    If header(0) = &H50 AndAlso header(1) = &H4B Then
                        ext = ".zip"
                    ElseIf header(0) = &H37 AndAlso header(1) = &H7A AndAlso header(2) = &HBC AndAlso header(3) = &HAF Then
                        ext = ".7z"
                    ElseIf header(0) = &H52 AndAlso header(1) = &H61 AndAlso header(2) = &H72 AndAlso header(3) = &H21 Then
                        ext = ".rar"
                    End If
                    DebugPrint($"[NEXUS调试] 检测到扩展名: '{ext}'", Color1.蓝色)
                    If ext <> "" Then
                        Dim newPath = 保存位置 & ext
                        fs.Close()
                        FileIO.FileSystem.RenameFile(保存位置, Path.GetFileName(newPath))
                        保存位置 = newPath
                        DebugPrint($"[NEXUS调试] 已重命名为: {保存位置}", Color1.蓝色)
                    Else
                        DebugPrint($"[NEXUS调试] 无法识别文件类型！前4字节不匹配任何已知格式", Color1.红色)
                    End If
                End Using
            End If
    ...
    Public Property 是否成功完成 As Boolean = False
        ...
        Try
            Using httpClient As New HttpClient()
                ...
                Using response As HttpResponseMessage = Await httpClient.GetAsync(URL, HttpCompletionOption.ResponseHeadersRead)
                    ...
                    End If
                    Dim cdnFileName As String = Nothing
                    DebugPrint($"[NEXUS调试] 响应头数量: {response.Headers.Count}", Color1.蓝色)
                    For Each h In response.Headers
                        DebugPrint($"[NEXUS调试] 响应头: {h.Key} = {String.Join(", ", h.Value)}", Color1.蓝色)
                    Next
                    For Each h In response.Content.Headers
                        DebugPrint($"[NEXUS调试] 内容头: {h.Key} = {String.Join(", ", h.Value)}", Color1.蓝色)
                    Next
                    If response.Headers.Contains("x-bz-file-name") Then
                        cdnFileName = response.Headers.GetValues("x-bz-file-name").FirstOrDefault()
                        DebugPrint($"[NEXUS调试] x-bz-file-name = {cdnFileName}", Color1.蓝色)
                    End If
                    If String.IsNullOrEmpty(cdnFileName) AndAlso response.Content.Headers.ContentDisposition IsNot Nothing AndAlso Not String.IsNullOrEmpty(response.Content.Headers.ContentDisposition.FileName) Then
                        cdnFileName = response.Content.Headers.ContentDisposition.FileName.Trim(""""c)
                        DebugPrint($"[NEXUS调试] ContentDisposition.FileName = {cdnFileName}", Color1.蓝色)
                    End If
                    If Not String.IsNullOrEmpty(cdnFileName) Then
                        保存位置 = Path.Combine(FileDir, Path.GetFileName(cdnFileName))
                    Else
                        Dim regex As New Regex("([^\/?]+)(?=\?|$)")
                        Dim match As Match = regex.Match(URL)
                        If match.Success Then
                            保存位置 = Path.Combine(FileDir, match.Value)
                        Else
                            Return "无法获取到文件名"
                            Exit Function
                        End If
    ...
    ''' <summary>解压后、安装前校验下载内容的 manifest UniqueID 与期望值是否一致，
    ''' 防止多文件模组（如 SVE 的各个农场文件）自动更新时误装其他附属文件</summary>
    Private Sub 校验下载内容UniqueID()
        If 设置_校验UniqueID = "" Then Exit Sub
        Dim 下载的UniqueID列表 As New List(Of String)
        For Each 解压文件夹 As String In 共享方法.SearchFolderWithoutSub(实际解压路径算起位置)
            ...
            End If
        Next
        If 下载的UniqueID列表.Count > 0 AndAlso Not 下载的UniqueID列表.Contains(设置_校验UniqueID) Then
            DebugPrint("[批量更新] UniqueID 校验失败：期望 " & 设置_校验UniqueID & "，实际 " & String.Join("、", 下载的UniqueID列表), Color1.红色)
            评估不通过操作("下载的文件包含的模组（" & String.Join("、", 下载的UniqueID列表) & "）与当前模组项（" & 设置_校验UniqueID & "）不匹配，已拒绝安装，请核对模组项对应的 NEXUS 文件")
        End If
    End Sub
    ...
    Public Sub 结束(Optional 是否要转选项卡 As Boolean = True, Optional 流程是否成功 As Boolean = True)
        报告流程结束(流程是否成功, If(流程是否成功, "安装完成", "更新流程未成功完成"))
        ' 更新内容刚写入模组项，立即事件驱动刷新管理模组列表的版本与状态显示
        管理模组.自动刷新当前列表数据()
