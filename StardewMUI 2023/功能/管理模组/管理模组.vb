...
Public Class 管理模组
    ...
    Public Shared Sub 初始化()
        ...
        AddHandler 管理模组的菜单.菜单项_打开链接.Click, Sub() Process.Start(New ProcessStartInfo(点击的链接) With {.UseShellExecute = True})
        ...
        ' 切换到管理模组选项卡时立即刷新一次数据，后续由定时器周期刷新
        AddHandler Form1.UiTabControl1.SelectedIndexChanged,
            Sub()
                If Form1.UiTabControl1.SelectedTab Is Form1.TabPage管理模组 Then 自动刷新当前列表数据()
            End Sub
    ...
    ''' <summary>数据自动刷新兜底定时器：事件驱动刷新（安装/卸载/更新完成时）为主，此定时器只负责
    ''' 兜底外部变更（用户手动改文件等），仅在管理模组选项卡可见时工作</summary>
    Private Shared WithEvents 数据自动刷新定时器 As New Timer With {.Interval = 60000, .Enabled = True}

    Private Shared Sub 数据自动刷新定时器_Tick(sender As Object, e As EventArgs) Handles 数据自动刷新定时器.Tick
        If Form1.UiTabControl1.SelectedTab IsNot Form1.TabPage管理模组 Then Exit Sub
        自动刷新当前列表数据()
    End Sub

    ''' <summary>立即重读当前分类模组项的 manifest，更新版本与安装状态列，并在选中单项时同步右侧详情。
    ''' 供安装/卸载完成、下载块结束等数据变化时事件驱动调用，也由定时器周期兜底</summary>
    Public Shared Sub 自动刷新当前列表数据()
        Try
            If Form1.ListView1.SelectedItems.Count = 1 AndAlso Form1.ListView2.Items.Count > 0 Then 刷新项列表数据()
            If Form1.ListView2.SelectedItems.Count = 1 Then 读取项信息并显示()
        Catch ex As Exception
            ' 自动刷新属于后台行为，任何异常静默跳过等待下一个周期
            DebugPrint("自动刷新列表数据失败：" & ex.Message, Color1.橙色)
        End Try
    End Sub
