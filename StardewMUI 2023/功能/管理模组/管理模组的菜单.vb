...
Public Class 管理模组的菜单
    ...
    ''' <summary>右键菜单专用的移动/删除/导出入口。不能复用顶部"项"菜单里的同名菜单项实例——
    ''' 一个 ToolStripMenuItem 只能挂在一个菜单里，挂进右键菜单就会从项菜单中消失</summary>
    Public Shared Property 右键_移动项 As New ToolStripMenuItem With {.Text = "移动项", .Image = My.Resources.移动}
    Public Shared Property 右键_删除项 As New ToolStripMenuItem With {.Text = "删除项", .Image = My.Resources.删除}
    Public Shared Property 右键_导出项 As New ToolStripMenuItem With {.Text = "导出项", .Image = My.Resources.上传}
    ...
    Public Shared Property 菜单项_更新键表 As New ToolStripMenuItem With {.Text = "更新键"}
    ...
    Public Shared Sub 添加项右键菜单的所有菜单项()
        ...
        项右键菜单.Items.Add(New ToolStripSeparator)
        项右键菜单.Items.Add(右键_移动项)
        项右键菜单.Items.Add(右键_删除项)
        项右键菜单.Items.Add(右键_导出项)
        ' 与顶部"项"菜单的对应条目执行完全相同的流程
        AddHandler 右键_移动项.Click, AddressOf 模组项操作.转移模组项
        AddHandler 右键_删除项.Click, AddressOf 模组项操作.删除模组项
        AddHandler 右键_导出项.Click, AddressOf 管理模组.导出模组项
    ...
    Public Shared Sub 添加菜单的触发()
        ...
        AddHandler Form1.ListView1.MouseDown, Sub(sender, e)
                                                  ...
                                                  End If
                                              End Sub
        AddHandler Form1.ListView2.MouseDown, Sub(sender, e)
                                                  If e.Button = MouseButtons.Right Then
                                                      ' MouseDown 阶段本地右键选中还没生效，手动把命中的项设为唯一选中项，保证更新键菜单取到正确的项
                                                      Dim 命中项 As ListViewItem = Form1.ListView2.GetItemAt(e.X, e.Y)
                                                      If 命中项 IsNot Nothing AndAlso Not 命中项.Selected Then
                                                      ...
                                                      If Form1.ListView2.SelectedItems.Count = 1 Then
                                                          菜单项_更新键表.Enabled = True
                                                          菜单项_更新键表.DropDown = 更新模组.生成更新地址表菜单()
                                                      Else
                                                          菜单项_更新键表.Enabled = False
                                                      End If
                                                      ' 移动/删除/导出同样只在右键命中（或本就选中）条目时可用
                                                      Dim 有选中项 As Boolean = Form1.ListView2.SelectedItems.Count >= 1
                                                      右键_移动项.Enabled = 有选中项
                                                      右键_删除项.Enabled = 有选中项
                                                      右键_导出项.Enabled = 有选中项
                                                      项右键菜单.Show(sender, e.X, e.Y)
                                                  End If
                                              End Sub
