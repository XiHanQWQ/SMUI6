...
Public Class GitAPI
    ...
    ''' <summary>按请求域名附加设置里填写的平台令牌：
    ''' GitHub（api.github.com / raw.githubusercontent.com）附带 Bearer 形式的 Personal Access Token，
    ''' 匿名限额 60 次/小时、携带后 5000 次/小时；
    ''' Gitee API（gitee.com/api/）附带 access_token 查询参数，提升限额并支持私有仓库。
    ''' 注意 GitHub 令牌发给 Gitee 会得到 401 Access token does not exist，两平台令牌不可混用</summary>
    Private Shared Sub 附加平台令牌(client As HttpClient, ByRef 请求地址 As String)
        Try
            If 请求地址.StartsWith("https://api.github.com") OrElse 请求地址.StartsWith("https://raw.githubusercontent.com") Then
                Dim GitHub令牌 As String = 设置.全局设置数据("GithubToken")
                If GitHub令牌 IsNot Nothing AndAlso GitHub令牌.Trim <> "" Then
                    client.DefaultRequestHeaders.Add("Authorization", "Bearer " & GitHub令牌.Trim)
                End If
            ElseIf 请求地址.StartsWith("https://gitee.com/api/") Then
                Dim Gitee令牌 As String = 设置.全局设置数据("GiteeToken")
                If Gitee令牌 IsNot Nothing AndAlso Gitee令牌.Trim <> "" Then
                    请求地址 &= If(请求地址.Contains("?"), "&", "?") & "access_token=" & Gitee令牌.Trim
                End If
            End If
        Catch ex As Exception
            DebugPrint("附加平台令牌失败：" & ex.Message, Color1.橙色)
        End Try
    End Sub
            ...
            Try
                ...
                If 令牌 <> "" Then
                    Select Case 目标平台
                        ...
                        Case 开源代码平台.GitHub
                            ' raw 端点的 ?token= 查询参数只接受临时令牌，PAT 走 Authorization 头（下方附加）
                    End Select
                End If
